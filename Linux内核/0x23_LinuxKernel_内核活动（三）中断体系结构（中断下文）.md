...
最早的Linux只提供了“bottom half”这种机制来实现下半部。它提供了一种静态创建的且由32个bottom halves阻证的链表。中断上文通过一个32位整数中的一个位来表示出哪个bottom half可以执行。每个BH在全局范围内同时同步。即便是分属于不同的处理器，也不允许任何两个BH同时执行。
...
> #### Note
> 
> SVC(Supervisor Call) was previously called SWI, Software Interrupt, and this name is still found in some documentation.
...
enum
{
	HI_SOFTIRQ = 0,		// 用于处理高优先级的任务，如网络设备的输入。
	TIMER_SOFTIRQ,		// 用于处理定时器事件。
	NET_TX_SOFTIRQ,		// 用于处理网络设备的输出。
	NET_RX_SOFTIRQ, 	// 用于处理网络设备的输入。
	BLOCK_SOFTIRQ,		// 用于处理块设备的 I/O。
	IRQ_POLL_SOFTIRQ, 	// 用于处理 I/O 设备的轮询。
	TASKLET_SOFTIRQ,	// 用于处理任务（tasklet），这是一种可以延迟执行的函数。
	SCHED_SOFTIRQ,		// 用于处理调度器的任务。
	HRTIMER_SOFTIRQ,	// 用于处理高分辨率定时器事件。
	RCU_SOFTIRQ			// 用于处理 RCU（Read-Copy-Update）的任务。
};
```
...
## 2.4 描述补充
在 Linux 内核中，软中断（softirq）的处理通常是在硬件中断处理程序返回后立即进行的，这部分工作是在硬件中断上下文中完成的，而不是在 ksoftirqd 守护进程中。

然而，如果系统负载过高，或者软中断的处理时间过长，这可能会导致软中断的处理被延迟。在这种情况下，ksoftirqd 守护进程就会被唤醒来处理这些未处理的软中断。

因此，我们可以说 ksoftirqd 守护进程是作为一种后备机制存在的，它处理那些不能立即处理的软中断。在正常情况下，大部分软中断的处理都是在硬件中断上下文中完成的，而不是在 ksoftirqd 守护进程中。
...
这段代码定义了一个名为 ksoftirqd 的函数，这是 Linux 内核中处理软中断（softirq）的守护进程。这个函数的主要任务是处理在当前 CPU 上挂起的软中断。

函数的主体是一个循环，它会一直运行，直到 kthread_should_stop() 返回 true。这通常在系统关闭或者需要停止 ksoftirqd 进程时发生。

在循环中，首先通过 preempt_disable() 禁止抢占，然后检查是否有挂起的软中断。如果没有，它会启用抢占并调用 schedule() 让出 CPU，然后再次禁止抢占。

如果有挂起的软中断，它会设置当前任务的状态为 TASK_RUNNING，然后进入一个内部循环，处理所有挂起的软中断。在这个内部循环中，它首先检查当前 CPU 是否已经离线。如果是，它会跳转到 wait_to_die 标签。否则，它会调用 do_softirq() 处理一个软中断，然后启用抢占，尝试进行条件调度，然后再次禁止抢占。这个内部循环会一直运行，直到没有更多的挂起的软中断。

在 wait_to_die 标签处，它会启用抢占，然后进入一个循环，等待 kthread_should_stop() 返回 true。

总的来说，这个函数的目的是处理在当前 CPU 上挂起的软中断。当没有软中断需要处理时，它会让出 CPU，让其他任务有机会运行。
...
- work_queue_struct：官方解释是对外部可见的workqueue；
- pool_workqueue：链接work_queue_struct  和worker_pool的中介，每个work_queue_struct 可以有多个worker_pool，而一个worker_pool只能属于一个work_queue_struct。
