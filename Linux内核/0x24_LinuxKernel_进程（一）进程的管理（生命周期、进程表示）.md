...
`fork()`可以创建当前**进程的一个副本且有独立的PID号码**（父进程和子进程只有PID号码不同）。Linux使用了一个众所周知的技术来使`fork`更高效，那就是写时复制（copy on write）操作。
...
`zombie 进程（僵尸进程）是指一个已经结束运行（终止），但其父进程尚未对其进行善后处理（获取终止状态）的进程`  
...
补充：  
`僵尸进程`（Zombie Process）是已经终止但其父进程尚未调用 wait() 或 waitpid() 获取其终止状态的进程。消除僵尸进程的常见方法包括以下几种：

方法一：让父进程调用 wait() 或 waitpid()
确保父进程正确地调用 wait() 或 waitpid() 来处理子进程的终止状态。这是最根本的方法。

方法二：终止父进程
如果父进程没有正确处理子进程的终止状态，可以通过终止父进程来消除僵尸进程。父进程终止后，僵尸进程会被 init 进程（PID 1）接管，init 进程会调用 wait() 来清理僵尸进程。
