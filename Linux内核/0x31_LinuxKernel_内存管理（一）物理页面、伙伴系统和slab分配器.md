...
## 高端内存 // 低端内存
```C
在Linux操作系统中，内存通常被分为低端内存（Low Memory）和高端内存（High Memory）。这些术语主要与内核的内存管理和地址空间布局有关。以下是对高端内存的详细解释：
高端内存（High Memory）
高端内存是指物理内存中位于内核直接映射范围之外的部分。在32位系统中，由于虚拟地址空间的限制，内核无法直接映射所有的物理内存，尤其是当物理内存大于大约1GB时。因此，物理内存被划分为低端内存和高端内存。
具体解释：

地址空间限制：
在32位系统中，虚拟地址空间总共只有4GB，其中通常1GB（从0xC0000000到0xFFFFFFFF）保留给内核使用，剩下的3GB（从0x00000000到0xBFFFFFFF）用于用户空间。
内核需要直接访问的内存必须映射到这1GB的内核地址空间中，但如果物理内存超过1GB，内核无法直接映射所有的物理内存。
低端内存（Low Memory）：
低端内存是指可以直接映射到内核地址空间的物理内存部分。通常，这部分内存位于物理地址的低端，例如在1GB以下的物理内存区域。
内核可以通过简单的地址转换直接访问低端内存。
高端内存（High Memory）：
高端内存是指超出内核直接映射范围的物理内存部分。对于32位系统来说，这通常是超过1GB的物理内存。
内核不能直接访问高端内存，需要通过动态映射（使用页表）来访问这些内存区域。
访问高端内存：
为了访问高端内存，内核需要使用临时映射（temporary mapping）或固定映射（permanent mapping）。
临时映射通常通过kmap()和kunmap()函数实现，这些函数在需要时将高端内存映射到内核地址空间中，并在使用完毕后取消映射。
固定映射则通过固定的内核地址映射高端内存，通常用于需要频繁访问的高端内存区域。
高端内存的使用场景
高端内存主要用于那些不需要频繁访问的内存区域，如：
页缓存（Page Cache）：用于缓存文件系统的数据。
高速缓存（Buffer Cache）：用于缓存块设备的数据。
用户空间内存分配：用户空间程序可以使用高端内存，但内核需要进行额外的映射操作。
高端内存的优缺点
优点：
允许系统使用更多的物理内存，即使在32位系统中也能支持超过1GB的物理内存。
提高了系统的内存利用率，特别是对于需要大量内存的应用程序。
缺点：
访问高端内存需要额外的映射操作，增加了内核的复杂性和开销。
由于需要频繁进行映射和取消映射操作，可能会导致性能下降。
64位系统中的高端内存
在64位系统中，虚拟地址空间的限制大大减少，内核可以直接映射大量的物理内存，因此高端内存的概念不再像在32位系统中那样重要。64位系统可以直接访问几乎所有的物理内存，减少了对高端内存管理的需求。
总结
高端内存是指超过内核直接映射范围的物理内存部分，主要在32位系统中使用。内核需要通过动态映射来访问高端内存，这增加了内存管理的复杂性，但也允许系统使用更多的物理内存。随着64位系统的普及，高端内存的概念变得不再那么重要。
```
...
伙伴系统的分配方式在Linux内存中也有分配小内存的机制在使用，比如kmalloc，他的原理是分配基于2的order次幂的内存块，然后也不需要分配页面。如图所示，这就很清晰一个经常问到的面试问题，**kmalloc和vmalloc有什么区别？**
```
我们经常听到的回答是vmalloc用于分配更大的内存，kmalloc分配小内存用的。实际上这个回答就不是很root casue，vmalloc直接touch伙伴系统，伙伴系统可以直接分配物理页面，而kmalloc是slab机制服务出的一层接口，用于高效分配小内存的，避免产生碎片。
总结
使用 kmalloc 时，内存块在物理上是连续的，适合小块内存分配和高性能需求的场景。
使用 vmalloc 时，内存块在虚拟地址空间中是连续的，但在物理内存中可能是分散的，
```
