...
##### 1. 初始状态
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-38-57-image.png)
...
##### 2. 切换到 `feature` 分支
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-39-06-image.png)
...
##### 3. 执行 `git rebase master`
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-39-13-image.png)
  
##### 4. 处理冲突情况
  
  在变基过程中，如果出现冲突，Git 会暂停变基操作。例如，在应用 `E` 提交（变成 `E'`）时出现冲突，图示如下：
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-39-21-image.png)
...
##### 5. 变基完成后的状态
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-39-29-image.png)
...
##### 1. 初始状态
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-14-18-image.png)
...
##### 2. 切换到目标分支
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-14-25-image.png)
...
##### 3. 执行 `git cherry-pick`
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-14-56-image.png)
  
##### 4. 多次 `cherry-pick` 场景
  
  如果我们还想把 `feature` 分支上的 `D` 提交也应用到 `master` 分支上，再次执行 `git cherry-pick D`。此时，Git 会在 `E'` 提交之后创建一个新的提交 `D'`。
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-15-04-image.png)
  
##### 5. 处理冲突情况
  
  在 `cherry-pick` 过程中，如果出现冲突，Git 会暂停操作并提示你解决冲突。假设在 `cherry-pick E` 时出现冲突，图示如下：
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-15-16-image.png)
...
##### 1. 基本分支切换场景
  ...
  **初始状态**
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-07-14-image.png)
  ...
  **执行 `git checkout feature`** 
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-07-32-image.png)
...
##### 2. 切换到指定提交（分离 HEAD 状态）
  ...
  **初始状态**
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-07-51-image.png)
  
  **执行 `git checkout commit B**
  
  当执行 `git checkout <commit B 的哈希值>` 时，`HEAD` 会直接指向指定的提交 `commit B`，此时处于分离 `HEAD` 状态，即 `HEAD` 不再指向任何分支。
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-08-07-image.png)
  
  **从分离 `HEAD` 状态创建新分支**
  
  如果在分离 `HEAD` 状态下进行了一些修改并想保存为一个新分支，可以执行 `git checkout -b new-branch`。这时会创建一个新分支 `new-branch`，并让 `HEAD` 指向这个新分支。
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-08-22-image.png)
...
##### 3. 使用 `git checkout` 恢复文件到指定版本
  ...
  **初始状态**
  ...
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-08-40-image.png)
  
  **执行 `git checkout commit A -- file.txt**
  
  执行 `git checkout <commit A 的哈希值> -- file.txt` 命令后，工作区的 `file.txt` 文件会恢复到 `commit A` 时的状态。
  
  ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-14-08-53-image.png)
...
#### 6.4.1 初始状态
   ...
   ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-13-25-59-image.png)
   
#### 6.4.2 执行 `git stash`
   
   当你执行 `git stash` 命令后，工作区和暂存区的修改会被保存到一个 stash 栈中，工作区和暂存区恢复到上一次提交时的干净状态。
   
   ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-11-12-35-image.png)
...
#### 6.4.4 执行 `git stash pop`
   ...
   ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-13-26-40-image.png)
   
#### 6.4.5 多次 stash 情况
   
   如果你多次执行 `git stash` 命令，stash 栈中会有多个记录，栈顶的记录是最近保存的。例如，执行了两次 `git stash` 后：
   
   ![](C:\Users\jiawei.peng\AppData\Roaming\marktext\images\2025-02-08-13-26-59-image.png)
